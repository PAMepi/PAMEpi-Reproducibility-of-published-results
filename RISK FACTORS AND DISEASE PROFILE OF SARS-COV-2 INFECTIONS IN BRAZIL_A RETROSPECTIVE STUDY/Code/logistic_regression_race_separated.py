...
# -*- coding: utf-8 -*-
"""
Verification of the missing ethnic/racial entries effects: logistic regression
only with ethnic/racial background and without using this variable (and not removing
...
col_names = ['MORTE', 'AGE_GRP', 'BDI_GRP', 'eUTI', 'NVACC', 'SEXO', 'TINT']
col_names = col_names + gr_risco 
X = data_init[col_names]
X = X.dropna()
X, labs = create_dummy(X, ['AGE_GRP',  'BDI_GRP', 'TINT'], ['D', 1, 0] )
out = X
...
with open('../Results/corr_logit_sem_raca.csv', 'w') as f:
...
sa.to_csv('../Results/logit_results_without_race.csv', index=False)
...
col_names = ['MORTE','RACA']
X = data_init[col_names]
X = X.dropna()
X = X[X.RACA != 'Desconhecida']
X, labs = create_dummy(X, ['RACA'], ['D'] )
out = X
Y = out['MORTE'].to_numpy()
X = out.drop(['MORTE'], axis=1).to_numpy(dtype=float).T
...
with open('../Results/corr_logit_so_raca_sd.csv', 'w') as f:
    f.write(' ,' + ', '.join(out.columns) + '\n')
    for i, line in enumerate(cor):
        f.write(out.columns[i] + ', ' + ', '.join([str(ele) for ele in line.tolist()])+ '\n')
...
sa = pd.DataFrame(saida)
sa.to_csv('../Results/logit_results_only_race_sd.csv', index=False)